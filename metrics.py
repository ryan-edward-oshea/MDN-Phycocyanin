...
@label('SpR')
def SpR(y, y_hat):
	''' Spearman R ranked correlation '''
	coef, p = spearmanr(y,y_hat) 
	return coef
...
def SpR_log(y, y_hat):
	''' Spearman R ranked correlation '''
	coef, p = spearmanr(np.log10(y),np.log10(y_hat)) 
	return coef
...
def performance(key, y, y_hat, metrics=[mdsa, sspb, slope, rmse, rmsle, mae, mape, leqznan]):
